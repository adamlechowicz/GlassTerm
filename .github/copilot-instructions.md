...
- **Window opacity is a global contrast backing.** `GlassOpacity.current` (persisted in `UserDefaults` key `"WindowOpacity"`, `0` = Glass … `1` = Translucent) drives a layer-backed `NSView` (`contrastBackingView`) — white in light mode, black in dark mode — layered **over the glass and under the terminal** (it doesn't replace the glass) to raise text contrast, updated via `updateContrastBacking()`. Opacity is deliberately kept **off** the SwiftUI path: rapidly reassigning `NSHostingView.rootView` while it hosts `.glassEffect()` crashes SwiftUI's AttributeGraph on macOS 26. The control is a slider in a **popover** opened from the View menu's "Glass Opacity…" item (`AppDelegate.showOpacityPopover` → `OpacityPopoverViewController`), not an `NSSlider` embedded directly in the menu (continuous sliders in menu custom views are unreliable).
- **`UserDefaults` keys (use these exact strings):** `"GlassTint"`, `"WindowOpacity"`, `"AlwaysDarkMode"`, `"TerminalFontName"`, `"TerminalFontSize"`, `"LogHostOutput"`.
...
- **ANSI palettes are appearance-adaptive:** `darkModeColors` / `lightModeColors` arrays of `SwiftTerm.Color`; `AlwaysDarkMode` forces the dark palette and a dark appearance on both window and glass. `isEffectiveDarkMode` (Always Dark Mode ∥ effective `.darkAqua`) is the single shared light/dark predicate — reuse it for any appearance-dependent logic.
- **Terminal color queries are answered to match the appearance.** Because the terminal background is transparent (`nativeBackgroundColor = .clear`), SwiftTerm would report black for `OSC 11;?` (alpha is dropped), so CLI apps (e.g. Copilot) misdetect *dark* mode and draw dark-theme selections that clash with GlassTerm's black light-mode text. `installColorQueryHandlers()` registers `OSC 10/11` handlers (via SwiftTerm's `registerOscHandler`, which wins over its built-ins) that answer *queries* with appearance-matched colors (light → bg white/fg black; dark → bg black/fg white) and ignore color *sets* to preserve transparency. Cursor color (OSC 12) is left to SwiftTerm. The `CSI ?996n` color-scheme DSR isn't answerable (no SwiftTerm CSI hook); apps that also use `OSC 11` (Copilot does) still work.
